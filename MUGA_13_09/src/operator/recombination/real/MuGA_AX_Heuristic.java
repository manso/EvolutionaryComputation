 ...
 * @author ZULU
 */
public class MuGA_AX_Heuristic extends Recombination {
...
//        children.addGenotype(i1);
//        children.addGenotype(i2);
        return localSearch(children);
    }

    public Population localSearch(Population pop) {
        pop.evaluate();
        Population improved = pop.getCleanCopie();
        RC_Individual best1 = (RC_Individual) pop.removeBestGenotype();
        RC_Individual best2 = (RC_Individual) pop.removeBestGenotype();
        while (true) {
            double[] v1 = best1.getValues();
            double[] v2 = best2.getValues();
            RC_Individual best3 = (RC_Individual) best1.getClone();
            double[] v3 = best3.getValues();
            for (int i = 0; i < v3.length; i++) {
                v3[i] = v1[i] + EXPAND * (v1[i] - v2[i]);
            }
            best3.setValues(v3);
            best3.evaluate();
            solver.addEvaluations(2);
            if (best3.compareTo(best1) >= 0) {
                best1 = best3;
            } else {
                break;
            }
        ...
        improved.addGenotype(best1);
        improved.addGenotype(best2);
        return improved;
...
//Automatic probability
    ...
    @Override
    public MuGA_AX_Heuristic getClone() {
        MuGA_AX_Heuristic clone = (MuGA_AX_Heuristic) super.getClone();
