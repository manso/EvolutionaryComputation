 ...
 * @author manso
 */
public class MuGA_Gauss_Expand extends Mutation {
    ...
    public Individual mutate(Individual ind, int copyNumber, double probability) {
        ...
        if (DYNAMIC > 0) {
            probability /= copyNumber;
        }
        //number of copie shrink interval
        double amplitud = (ind.getDimension() * EXPAND) * copyNumber;
        //for all genes
    ...
    @Override
    public MuGA_Gauss_Expand getClone() {
        MuGA_Gauss_Expand clone = (MuGA_Gauss_Expand) super.getClone();
